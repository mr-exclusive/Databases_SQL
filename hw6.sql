...
BEGIN
	...
	RETURN (SELECT 
		CASE
			WHEN @cur_time BETWEEN TIME('06:00:00') AND TIME('11:59:59') THEN 'Доброе утро'
			WHEN @cur_time BETWEEN TIME('12:00:00') AND TIME('17:59:59') THEN 'Добрый день'
            WHEN @cur_time BETWEEN TIME('18:00:00') AND TIME('23:59:59') THEN 'Добрый вечер'
            WHEN @cur_time BETWEEN TIME('00:00:00') AND TIME('05:59:59') THEN 'Доброй ночи'
			ELSE 'неопределено'
		END);
END//
DELIMITER ;
...
-- 3) (по желанию)* Создайте таблицу logs типа Archive. Пусть при каждом создании записи в таблицах users, communities и messages в таблицу logs 
...
DROP TRIGGER IF EXISTS log_community_insert;
DROP TRIGGER IF EXISTS log_message_insert;
DELIMITER //
CREATE TRIGGER log_user_insert AFTER INSERT ON users
FOR EACH ROW
BEGIN
	INSERT INTO log(tbl_name, rec_id)
	VALUES ('users', NEW.id);
END//
...
VALUES 
('Иван', 'Диванов', 'ivan_2023@mail.ru'),
('Петр', 'Петров', 'retya_2023@mail.ru');
...
-- 4) Создайте функцию, которая принимает кол-во сек и формат их в кол-во дней часов.
-- Пример: 123456 ->'1 days 10 hours 17 minutes 36 seconds '

DROP FUNCTION IF EXISTS format_seconds;
DELIMITER //
CREATE FUNCTION format_seconds(seconds BIGINT)
RETURNS VARCHAR(100) NO SQL -- READS SQL DATA 
BEGIN
	RETURN CONCAT(
	  FLOOR(TIME_FORMAT(SEC_TO_TIME(seconds), '%H') / 24), ' days ',
	  MOD(TIME_FORMAT(SEC_TO_TIME(seconds), '%H'), 24), ' hours ',
...
-- 5) Выведите только четные числа от 1 до 10.
-- Пример: 2,4,6,8,10

DROP PROCEDURE IF EXISTS show_even_numbers;
DELIMITER //
CREATE PROCEDURE show_even_numbers(n INT)
BEGIN
