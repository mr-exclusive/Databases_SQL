...
SELECT
	*,
    DENSE_RANK() OVER(ORDER BY tbl_users_msg.msg_count DESC) AS raiting
FROM (
	SELECT 
...
SELECT
	*,
    -- LAG(created_at, 1, created_at) OVER(ORDER BY created_at) AS prev_msg_time,
    SEC_TO_TIME(TIMESTAMPDIFF(SECOND, LAG(created_at, 1, created_at) OVER(ORDER BY created_at), created_at)) AS time_diff
FROM messages
ORDER BY created_at;