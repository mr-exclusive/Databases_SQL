...
SELECT 
    COUNT(l.id) AS total_likes_under_age_12
FROM likes AS l
...
-- Ищем пользователя который больше всего написал сообщений пользователю - 'Reuben Nienow'
SELECT 
	(SELECT 
		CONCAT(users.firstname, ' ', users.lastname) 
	FROM users 
    WHERE users.id = m.from_user_id) AS full_name,
    COUNT(*) AS msg_sent
FROM messages AS m
JOIN friend_requests AS fq ON m.from_user_id = fq.initiator_user_id OR m.from_user_id = fq.target_user_id
JOIN users AS u ON m.to_user_id = u.id
WHERE
    fq.`status` = 'approved' AND
    u.firstname = 'Reuben' AND 
    u.lastname = 'Nienow'
